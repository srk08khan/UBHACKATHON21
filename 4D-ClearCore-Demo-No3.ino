...
#define SerialPort ConnectorCOM1      // ClearCore UART Port, connected to 4D Display COM1
...
void setup()
{
  ...
  while (!genie.Begin(Serial1)); // Set up Genie to use Serial port using the Arduino serial class, but also returns if the Display has responded and is online
...
/************* MOTOR ***************/
    ...
    if(!AxisStopRun[i])
    {
      ...
      clients[i].print(motorConnectorNames[i]); clients[i].println(" Stopped");
...
void telnet(void)
{
  ...
  for (byte i=0; i < 8; i++) {
    if (clients[i] && clients[i].available() > 0) {
      ...
      clients[i].print("I got some data");
